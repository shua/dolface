...
void read_dol_segments(ELF_map *map, const char *dol)
{
    int read, i;
    DOL_hdr hdr;
    
    if(verbosity >= 2)
        fprintf(stderr, "Reading DOL file...\n");
    ...
    if(read != 1)
        ferrordie(map->dol, "reading DOL header");
    ...
    if(!tsec)
        perrordie("No text sections");
    for(i=0;i<11;++i) {
        if(hdr.data_size[i] != 0)
            dsec = i+1;
    }
    if(!dsec)
        perrordie("No data sections");
    if(verbosity >= 2)
        fprintf(stderr, "%d text sections, and %d data sections in dol\n", tsec, dsec);
...
void write_elf(ELF_map *map, const char *elf)
{
    FILE *elff;
    int written;
    int i;
...
int main(int argc, char** argv)
{
    ...
    const char *dol_file = arg[0];
    const char *elf_file = arg[1];
