...
typedef void (*DatPrintFun)(DatStructVar*,uint8_t*,DatStructVar);
typedef int (*DatTravFun)(DatStructVar*, uint8_t*,offtype*);
typedef struct {
    char* name;
    unsigned int size;
    DatStructVar* vars;
...
offtype* lfind(offtype* key, offtype* base, size_t _s, size_t size, int(*compar)(const void*, const void*)) {
    int i;
    for(i=0; i<size; ++i) {
...
offtype* otbsearch(offtype* key, offtype* base, size_t _s, size_t size, int(*compar)(const void*, const void*)) {
    int i;
    while(size) {
        i=(size/2);
    ...
    for(i=u;i<j && j<z; ++i) {
        ...
        if(match != NULL) {
            if(match->t != DatNULL && match->t != ots[i].t) {
                fprintf(stderr, "%08x is double-defined: %s, %s %8x %8x\n", match->o-0x20, ddefs[match->t].name, ddefs[ots[i].t].name, match->p-0x20, ots[i].p-0x20);
                continue;
            } else {
                t = match->v;
                *match = ots[i];
            ...
            fprintf(stderr, "%08x weird offset not found in relocation table or root nodes: %s, %s %8x\n", ots[i].o-0x20, ddefs[ots[i].t].name, ddefs[ots[i].pt].name, ots[i].p-0x20);
            match = ots+i;
        }

        if(match->o-0x20 > hdr->datasz) {
            fprintf(stderr, "Way too much offset t:%s\tz:%-8x o:%-8x pt:%s\tp:%-8x\n", ddefs[match->t].name, hdr->datasz, match->o-0x20, ddefs[match->pt].name, match->p-0x20);
            match->v=1;
            continue;
        }
#ifdef DEBUG
        printf("%-11s %8x %8x %d", ddefs[match->t].name, match->o-0x20, match->p-0x20, match->v);
#endif
        while(match->v!=1) {
            /* too cute? using offset.v as neg counter for arrays, and pos counter for lists
             * should probably just use another counter 
             * if I have some other set (list,array,...), I'll have to because ints only go two ways */
            t=j+ddefs[match->t].trav(
                    ddefs[match->t].vars,
                    buf+match->o+(((match->v<1)?-(match->v):(match->v-2))*ddefs[match->t].size), 
                    ots+j);
#ifdef DEBUG
            while(j<t) { 
                if(match->o % 4) fprintf(stderr, "Weird offset %s: %8x %8x\n", ddefs[match->t].name, match->o-0x20, match->p-0x20);
                ots[j].p = match->o;
                ots[j].pt = match->t;
                ++j;
            }
...
#endif
            ++(match->v);
            /* checking for 0 termination of list */
            if(match->v>1 && !(*(uint32_t*)(buf+match->o+((match->v-2)*ddefs[match->t].size)))) {
                match->v=1;
            }
...
pfoffs(char* file_s) {
    ...
    for(i=0; i<ddata.otm; ++i) {
        ...
        printf("%08x %s\n", ddata.offtypes[i].o, ddefs[ddata.offtypes[i].t].name);
    }
...
pfroot(char* file_s) {
    ...
    if(hdr[3]) {
        ...
        ddefs[DatRoot].print(ddefs[DatRoot].vars, buf, (DatStructVar){ DatHdr, strt, hdr[3] } );
    }
    if(hdr[4]) {
        printf("-- %08x %s --\n", root+i*ddefs[DatRoot].size, ddefs[DatRoot].name);
        ddefs[DatRoot].print(ddefs[DatRoot].vars, buf+(hdr[3]*ddefs[DatRoot].size), (DatStructVar){ DatHdr, strt, hdr[4] } );
    }
...
pfstruct(char* file_s, uint32_t type, uint32_t off) {
    ...
    ddefs[type].print(ddefs[type].vars, buf, (DatStructVar){ 0 });
